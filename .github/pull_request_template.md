## Summary

## Security Impact

- [ ] No security impact
- [ ] Security-relevant change

Security-relevant areas include authentication, authorization, MFA, audit
logging, session/token handling, Docker exposure, TLS/security headers,
dependency updates, AI provider connectivity, and documentation that affects
deployment safety.

## Documentation

- [ ] User-facing docs updated if behavior changed
- [ ] Security/deployment docs updated if controls or operational guidance changed
- [ ] No generated secrets, certificates, logs, database dumps, or build artifacts included

## Verification

- [ ] Backend build passes
- [ ] Backend tests pass
- [ ] Frontend build passes
- [ ] Frontend tests pass, when relevant
- [ ] Frontend lint reviewed, when relevant
- [ ] Dependency audit reviewed
- [ ] Docker Compose config/build reviewed, when relevant
